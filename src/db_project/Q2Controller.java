...
public class Q2Controller implements Initializable {
    ...
    public void initialize(URL url, ResourceBundle rb) 
 {
        try {
            ...
            String q = "select top 5 m.Client_ID,c.name ,sum(t.Price)as Total_SUM\n" +
"from MEMBER m,CLIENT c,TICKET t\n" +
"where m.Client_ID=c.Client_ID and m.Client_ID=t.Client_ID\n" +
"group by c.name,m.Client_ID\n" +
"order by sum(t.Price) desc\n" +
"";
            ResultSet rs = st.executeQuery(q);
            while (rs.next()) {
                list.add(new Table(rs.getString(1), rs.getString(2), rs.getInt(3)));
            }
